...
const latest = computed(() => stock_data.value.at(-1) || {});
...
const fetchData = async () => {
  try {
    const response = await fetch('http://127.0.0.1:5000/googl');
    if (!response.ok) throw new Error('Failed to fetch data');

    stock_data.value = await response.json();
    stock_data.value.sort((a, b) => new Date(a.record_date) - new Date(b.record_date));

    open.value = stock_data.value.map(d => d.open);
    close.value = stock_data.value.map(d => d.close);
    high.value = stock_data.value.map(d => d.high);
    low.value = stock_data.value.map(d => d.low);
    labels.value = stock_data.value.map(d => new Date(d.record_date).toISOString().split('T')[0]);
