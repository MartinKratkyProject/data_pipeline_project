...
const getArrowColor = (current, prev) => {
  if (prev === undefined || current === undefined) return 'gray';
...
const fetchData = async () => {
  error.value = null;
  try {
    const response = await fetch('http://127.0.0.1:5000/tsla');
    if (!response.ok) throw new Error('Failed to fetch data');

    const data = await response.json();
    data.sort((a, b) => new Date(a.record_date) - new Date(b.record_date));
    stock_data.value = data;

    open.value = data.map((d) => d.open);
    close.value = data.map((d) => d.close);
    high.value = data.map((d) => d.high);
    low.value = data.map((d) => d.low);
    labels.value = data.map((d) => new Date(d.record_date).toISOString().split('T')[0]);

    const minPrice = Math.min(...low.value);
    const maxPrice = Math.max(...high.value);
